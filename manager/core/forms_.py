...
class ConsumableGroupForm(forms.ModelForm):
    class Meta:
        model = ConsumableGroup
        fields = ['group_name']
        labels = {'group_name': 'Название группы'}


class NormConsumableCreateForm(forms.ModelForm):
    class Meta:
        model = NormConsumable
        fields = ['consumable_type', 'quantity', 'mu']
        labels = {
            'consumable_type': 'Тип расходника',
            'quantity': 'Расход в месяц',
            'mu': 'Единица измерения'
        }


class ConsumableIssueForm(forms.ModelForm):
    class Meta:
        model = ConsumableIssue
        fields = ['norm', 'initial_quantity', 'issue_date']
        widgets = {
            'issue_date': forms.DateInput(attrs={'type': 'date'}),
            'initial_quantity': forms.NumberInput(attrs={'step': '0.01'})
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if employee and employee.consumable_group:
            self.fields['norm'].queryset = NormConsumable.objects.filter(
                consumable_group=employee.consumable_group
            )